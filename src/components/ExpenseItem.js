...
export default function ExpenseItem(props) {
  ...
  function handleEdit(){
    ...
    let newAmount = amountInput.current.value.trim();
    props.itemEdit(newTitle, newAmount, props.data.id);
  }

  React.useEffect(() => {
    window.addEventListener("click", function(event){
      if(editMode && event.target !== titleInput.current && event.target !== amountInput.current && event.target !== title.current && event.target !== amount.current  && titleInput.current !== null && amountInput.current !== null && titleInput.current.value.trim().length > 0 && amountInput.current.value.trim().length > 0) {
        setEditMode(false);
      } 
    })
  
    document.addEventListener("keypress", function(event){
      if(event.which === 13 && editMode && titleInput.current !== null && amountInput.current !== null && titleInput.current.value.trim().length > 0 && amountInput.current.value.trim().length > 0 ) {
        setEditMode(false);
      }
    })
    if(!editMode) {
    ...
    <div className="expenseItem p-2 bg-gradient shadow-sm">
      ...
      <div className="expenseItemDescription p-2">
        ...
        <div className="amount-delete-wrapper fs-5">
          ...
          <input ref={amountInput} type="number" min="0.01" step="0.01" value={props.data.amount} className="expenseItemAmountInput fs-6" onChange={handleEdit}/>
